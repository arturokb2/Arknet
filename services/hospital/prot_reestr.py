        ...
        for e in err:
            for k,v in e.items():
                ...
                if k == 't005':
                    ...
                    doc.add_paragraph(
                        f"{v.sluchay.nib} {v.patient.fam} {v.patient.im} {v.patient.datr.strftime('%d.%m.%Y')} {o} - нет табельного леч.врача")
                elif k == 'err_text':
                    ...
                    er = set(er)
                    er = ','.join(er)
                    if v.sluchay.vds.vds.kod in ['Д','5']:
                        if er != 'Заполните I.4.1.Импланты!':
                            doc.add_paragraph(
                                f"{v.sluchay.nib} {v.patient.fam} {v.patient.im} {v.patient.datr.strftime('%d.%m.%Y')} {o} - {er} ")
                    else:
                        doc.add_paragraph(f"{v.sluchay.nib} {v.patient.fam} {v.patient.im} {v.patient.datr.strftime('%d.%m.%Y')} {o} - {er} ")

                elif k == 'n_pol':
                    doc.add_paragraph(
                        f"{v.sluchay.nib} {v.patient.fam} {v.patient.im} {v.patient.datr.strftime('%d.%m.%Y')} {o} - не задан пол")
                elif k == 'n_datr_d':
                    doc.add_paragraph(
                        f"{v.sluchay.nib} {v.patient.fam} {v.patient.im} {v.patient.datr.strftime('%d.%m.%Y')} {o} - не задана дата рождения")
                elif k == 'n_datr_m':
                    doc.add_paragraph(
                        f"{v.sluchay.nib} {v.patient.fam} {v.patient.im} {v.patient.datr.strftime('%d.%m.%Y')} {o} - не задана дата рождения")
                elif k == 'n_datr':
