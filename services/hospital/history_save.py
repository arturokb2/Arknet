...
class Save():
    ...
    def save(self):
        ...
        try:
            if self.user.ws.kod == 1:
                sluchay.otd = otde.objects.get(naim=str(history['otd']).strip(),tipe=1,dateend=None) if history['otd'] != None and history['otd'] != '' else None
            else:
                sluchay.otd = otde.objects.get(naim=str(history['otd']).strip(),tipe=2, dateend=None) if history['otd'] != None and history['otd'] != '' else None
        ...
        except group_kc_dkk.MultipleObjectsReturned:
            sluchay.oopkk = group_kc_dkk.objects.filter(kod=str(history['oopkk']).strip(),ksg_1=history['ksg_osn_all']).first()


        if history['ksg_osn_2_helper'] is not None:
            try:
                sluchay.ksg_osn2 = group_kc_group.objects.get(id=history['ksg_osn_2_helper'],dateend=None) if history['ksg_osn_2_helper'] != '' and history['ksg_osn_2_helper'] != None else None
            except group_kc_group.DoesNotExist:
                sluchay.ksg_osn2 = None
        else:
            try:
                g = group_kc_group.objects.values('id').filter(code_usl_kz=history['ksg_osn2'],dateend=None)[:1][0]
                sluchay.ksg_osn2 = group_kc_group.objects.get(id=g['id'])
            except:
                sluchay.ksg_osn2 = None
        
        try:
            sluchay.dskz3 = Ds.objects.get(kod=str(history['dskz3']['kod']).strip(), dateend=None) if history['dskz3']['kod'] != None and \
                                                                                                    history['dskz3']['kod'] != '' else None
        except Ds.DoesNotExist:
            sluchay.dskz3 = None
        

        try:
            sluchay.oopkk2 = group_kc_dkk.objects.get(kod=str(history['oopkk2']).strip(), ksg_1=history['ksg_osn2_all'],
                                                     dateend=None)
        except group_kc_dkk.DoesNotExist:
            sluchay.oopkk2 = None

        except group_kc_dkk.MultipleObjectsReturned:
            sluchay.oopkk2 = group_kc_dkk.objects.filter(kod=str(history['oopkk2']).strip(),ksg_1=history['ksg_osn2_all']).first()
        ...
        try:
            sluchay.code_usl2 = T003.objects.get(kod=str(history['code_usl2']).strip(),vidpom=1,usl_ok=1,idsp=33, dateend=None) if history['code_usl2'] != None and \
                                                                                                       history['code_usl2'] != None != '' else None
        except T003.DoesNotExist:
            sluchay.code_usl2 = None
        except T003.MultipleObjectsReturned:
            sluchay.code_usl2 = None
        ...
        try:
            if self.user.ws.kod == 1:
                sluchay.otd_y = otde.objects.get(naim=str(history['otd_y']).strip(), dateend=None,tipe=1) if history['otd_y'] != None else None
            else:
                sluchay.otd_y = otde.objects.get(naim=str(history['otd_y']).strip(), dateend=None,tipe=2) if history['otd_y'] != None else None
        ...
        sluchay.save()
        vds = str(history['vds']).strip()
        if vds in ['личные средства пациента','договор с юридич.лицами','ДМС','Фонд социал-го страхован.']:
            sluchay.ksg_osn = None
            sluchay.save()
        ...
        try:
            d = {}
            d['dskz'] = sluchay.dskz3.kod if sluchay.dskz3 else ''
            d['dsc'] = ''
            d['ds_osl'] = ''
            d['ksg_osn'] = history['ksg_osn2']
            d['ksg_osn_all'] = history['ksg_osn2_all']
            d['pol'] = str(patient.pol.id_pol) if patient.pol else ''
            dr = patient.datr.split('-')
            d['datr'] = f'{dr[2]}-{dr[1]}-{dr[0]}'
            d['oper_osn'] = ''
            d['oopkk'] = history['oopkk2']
            d['code_usl'] = history['code_usl2']
            dv = sluchay.datv.split('-')
            d['datv'] = f'{dv[2]}-{dv[1]}-{dv[0][2:]}'
            dp = sluchay.datp.split('-')
            d['datp'] = f'{dp[2]}-{dp[1]}-{dp[0][2:]}'
        ...
        try:
            if sluchay.ksg_osn2 == None:
                r = requests.get('http://arknet.okb2-tmn.ru/hospital/search/check_ksg', params=d)
                r_id = r.json()['r']
                try:
                    sluchay.ksg_osn2 = group_kc_group.objects.get(id=r_id)
                except group_kc_group.DoesNotExist:
                    sluchay.ksg_osn2 = None
                except:
                    pass
    ...
    def update_vb_s(self, vb_s, data):
        ...
        try:
            if self.user.ws.kod == 1:
                vb_s.potd = otde.objects.get(naim=str(data['potd']), dateend=None,tipe=1) if ((data['potd'] != None) and (data['potd'] != "")) else None
            else:
                vb_s.potd = otde.objects.get(naim=str(data['potd']), dateend=None,tipe=2) if ((data['potd'] != None) and (data['potd'] != "")) else None
