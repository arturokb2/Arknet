...
class Patients:
    ...
    def get_sluchays(self):
        ...
        #     sluchay_list = Sluchay.objects.values('id').filter(datv__range=(self._date_1, self._date_2),otd__tipe=1,update_user__ws=1).exclude(update_user=None)
        # else:
        #     sluchay_list = Sluchay.objects.values('id').filter(datv__range=(self._date_1, self._date_2),otd__tipe=2,update_user__ws=2).exclude(update_user=None)
            sluchay_list = Sluchay.objects.values('id').filter(datv__range=(self._date_1, self._date_2),update_user__ws=1).exclude(update_user=None)
        else:
            sluchay_list = Sluchay.objects.values('id').filter(datv__range=(self._date_1, self._date_2),update_user__ws=2).exclude(update_user=None)
        self.patients = []
        # threads = []
...
class PatientsData:
    ...
    def sluchays(self,cah=False,update_user=True):
        if cah:
            ...
            #     else:
            #         sluchay_list = Sluchay.objects.values('id').filter(datv__range=(self.date_1, self.date_2),otd__tipe=1)
            # else:
            #     if update_user:
            #         sluchay_list = Sluchay.objects.values('id').filter(datv__range=(self.date_1, self.date_2),otd__tipe=2).exclude(update_user=None)
            #     else:
            #         sluchay_list = Sluchay.objects.values('id').filter(datv__range=(self.date_1, self.date_2),otd__tipe=2)
            if self.user.statistics_type.id == 2:
                sluchay_list = Sluchay.objects.values('id').filter(datv__range=(self.date_1, self.date_2),update_user__ws=1).exclude(update_user=None)
            else:
                sluchay_list = Sluchay.objects.values('id').filter(datv__range=(self.date_1, self.date_2),update_user__ws=2).exclude(update_user=None)
        ...
        for s in sluchay_list:
            ...
            self.get_data(s['id'])
            # except:
            #     sl = Sluchay.objects.get(id=s['id'])
            #     # print(sl.nib)
    ...
    def get_data(self,id):
        ...
        try:
            ...
            self.patients.append(
                PatientYear().set_patient(patient)\
