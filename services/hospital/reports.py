...
class Reports(Patients):
    ...
    def filter(self,*args,**kwargs):
        ...
        for sluchay in sluchays:
            ...
            if filters.get('prof', None) != None:
                if sluchay['le_vr'] != None :
                    if filters.get('prof')['prof'] != '':
                        ...
                        if f_ == False: continue
            if filters.get('ymer_ymer', None) != None:
                if filters.get('ymer_ymer')['ymer_ymer'] != '':
                    f_ = sluchay['sluchay'].icx.id_iz in [105,106] if sluchay['sluchay'].icx != None else False
                    if f_ == False: continue
                    # f_ = sluchay['le_vr'].prof_k.k_prname == filters.get('prof')['prof'] if sluchay['le_vr'].prof_k != None else False
                    # if f_ == False: continue
            ...
            if filters.get('dsc', None) != None:
                dsc_1,dsc_2 = filters.get('dsc').values()
                if sluchay['sluchay'].dsc:
                    if dsc_1 != '' and dsc_2 != '':
                        ds_list = self._ds_list(dsc_1, dsc_2)
                        if ds_list:
                            if sluchay['sluchay'].dsc.kod not in ds_list:
                                continue
                    else:
                        continue
                else:
            ...
            if filters.get('dspat', None) != None:
                dspat_1,dspat_2 = filters.get('dspat').values()
                if sluchay['sluchay'].dspat != None:
                    if dspat_1 != '' and dspat_2 != '':
                        ds_list = self._ds_list(dspat_1, dspat_2)
                        if ds_list:
                            if sluchay['sluchay'].dspat.kod not in ds_list:
                                continue
                    else:
                        continue
                else:
            ...
            if filters.get('ds_osl', None) != None:
                ds_osl_1,ds_osl_2 = filters.get('ds_osl').values()
                if sluchay['sluchay'].ds_osl != None:
                    if ds_osl_1 != '' and ds_osl_2 != '':
                        ds_list = self._ds_list(ds_osl_1, ds_osl_2)
                        if ds_list:
                            if sluchay['sluchay'].ds_osl.kod not in ds_list:
                                continue
                    else:
                        continue
                else:
                    continue
            if filters.get('ds_onk', None) != None:
                ds_onk_1,ds_onk_2 = filters.get('ds_onk').values()
                if sluchay['sluchay'].dson != None:
                    if ds_onk_1 != '' and ds_onk_2 != '':
                        ds_list = self._ds_list(ds_onk_1, ds_onk_2)
                        if ds_list:
                            if sluchay['sluchay'].dson.kod not in ds_list:
                                continue
                    else:
                        continue
                else:
            ...
            if filters.get('kod_op',None) != None:
                if sluchay['oper'] != None:
                    if filters.get('kod_op')['kod_op'] != '':
                        opers_list = []
                        if len(sluchay['oper'])>0:
                            for oper in sluchay['oper']:
                                    if oper.kod_op:
                                        opers_list.append(oper.kod_op.kod)
                            else:
                                f_ = filters.get('kod_op')['kod_op'] in opers_list
                                if f_ == False: continue
                        else:
                            continue
                    else:
            ...
            if filters.get('rasxp',None) != None:
                rasxp =  filters.get('rasxp')
                if rasxp['rasxp'] == 'да':
    ...
    def _age_group(self,*args):
        vs,nvs,datr,datp,age_group = args
        try:
            vs = datp.year - datr.year
        except:
            vs = 0
...
class PredOperPlkdSpecification(Specification):
    def is_satisfied(self,item):
        if item.sluchay.oper.count() != 0 and item.sluchay.goc != None and item.sluchay.goc.tip_name == 'Плановая':
            oper = item.sluchay.oper.filter(pop=True)
            if oper.count() > 0:
...
class AgeSpecification(Specification):
    def is_satisfied(self, item):
        datp_year = item.sluchay.datp.year if item.sluchay.datp else 0
        datr_year = item.patient.datr.year if item.patient.datr else 0
        year = datp_year - datr_year
        if year > 0:
            return year
        return 0
        # try:
        #     return  item.sluchay.datp.year - item.patient.datr.year
        # except:
...
class IsfinKastSpecification(Specification):
    ...
    def is_satisfied(self, item):
        ...
        if self.p == None:
            ...
            if vds.vds and vds.vds.kod == '1' and vds.nctp != '' and vds.t_pol != None:
                if vds.ctkom and vds.ctkom.num_filter != None:
                    if item.patient.m_roj.find('Тюмень') != -1 or item.patient.m_roj.find('ТЮМЕНЬ') != -1:
                        if self.typ == 'all':
                            return 1
                        elif self.typ == 'kd':
                            return item.le_vr.kd
            ...
            if vds.vds and vds.vds.kod == '1':
                # if vds.ctkom and vds.ctkom.num_filter == None:
                # # if vds.ctkom and vds.ctkom.naim.find('ТЮМЕН') == -1:
                if item.patient.m_roj.find('Тюмень') == -1 and item.patient.m_roj.find('ТЮМЕНЬ') == -1:
                    if vds.t_pol != None:
                        # print(item.sluchay.nib)
                        if self.typ == 'all':
                            return 1
            ...
            if vds.vds and vds.vds.kod in ['2','A']:
                if self.typ == 'all':
                    return 1
                elif self.typ == 'kd':
                    return item.le_vr.kd
