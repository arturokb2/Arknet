...
from services.hospital.reference_reports import get_opers
...
class Create(CreateReestr):
    ...
    def create(self):
        ...
        # if (self.type_reestr == '' or self.type_reestr in ['kard','implants']) and self.his == '':
        #     self.patients = PatientsData(self.date_1, self.date_2, self.user)
        #     if self.type_reestr == 'implants':
        #         self.patients.sluchays(update_user=False)
        #     else:
        ...
        #                 temp.append(t)
        #         elif self.type_reestr == 'implants':
        #             if t.sluchay.otd and t.sluchay.otd.naim not in ['КАРДИОЛОГИЧЕСКОЕ']:
        #                 oper = self.oper_pop(t)
        #                 if oper is not None:
        ...
        # else:
        #     async_to_sync(get_channel_layer().group_send)(self.user_group_name, {'type': 'progress', 'text': 100})
        self.vid_vme_dem = ['A18.05.002','A18.05.002.001','A18.05.002.002','A18.05.002.003',
                            'A18.05.002.005','A18.05.002.004','A18.05.011','A18.05.011.001',
                            'A18.05.011.002']
        if self.type_reestr != 'Реестр по выбранным историям.dbf':
            self.patients = PatientsData(self.date_1, self.date_2, self.user)
            self.patients.sluchays()
            for p in self.patients.patients:
                vds = p.sluchay.vds.vds.kod if p.sluchay.vds.vds else None
                if self.type_reestr == 'ОСН.dbf':
                    if vds == '1':
                        temp.append(p)
                elif self.type_reestr == 'БПО.dbf':
                    if vds == 'Д':
                        temp.append(p)
                elif self.type_reestr == 'ВСП.dbf':
                    if vds == '5':
                        temp.append(p)
                elif self.type_reestr == 'Реестр.dbf':
                    if vds in ['1','Д','5']:
                        temp.append(p)
        else:
            his = [h.strip() for h in self.his.split(',') if h != '' and len(h.strip()) == 10]
    ...
    def oper_pop(self,pat):
        ...
        if pop.count() > 0:
            return pop[0]
        return None
    def tarif_sp(self,pat,ksg2=None):
        if pat.sluchay.tip_oms == '1':
            if ksg2 is None:
                ...
                if pat.sluchay.code_usl2 != None:
                    try:
                        return Tarif.objects.get(Code_usl=pat.sluchay.code_usl2.kod, Ur='3.0', KOEF_D='1.10500',
                                                 dateend=None)
                    except:
                        return None
                else:
                    t003 = None
                    try:
                        t003 = T003.objects.get(vidpom=1, usl_ok=1, dateend=None,
                                                code_usl_kz=pat.sluchay.ksg_osn2.code_usl_kz)
                    except:
                        t003 = None
    ...
    def novor_sp(self,pat,err):
        ...
        if (pat.sluchay.datp - pat.patient.datr).days <= 28:
            ...
            novor = ''
            if pat.patient.pol != None:
                novor += str(pat.patient.pol.id_pol)
            else:
                novor += ''
                err['n_pol'] = pat
            if pat.patient.datr != None:
                day = str(pat.patient.datr.day)
                if len(day) == 1:
                    day = f'0{day}'
                novor += day
            else:
                ...
                err['n_datr_d'] = pat
            if pat.patient.datr != None:
                month = str(pat.patient.datr.month)
                if len(month) == 1:
                    month = f'0{month}'
                novor += month
            else:
            ...
            if pat.patient.datr != None:
                datr = str(pat.patient.datr.year)[2:]
                novor += datr
            else:
                novor += ''
                err['n_datr'] = pat
    ...
    def for_pom_sp(self,pat):
        if pat.sluchay.code_usl != None and pat.sluchay.code_usl.kod == '1.1.3.150':
            ...
            return None
    def det_sp(self,pat):
        if pat.sluchay.iddokt:
            v002 = int(pat.sluchay.iddokt.v002) if pat.sluchay.iddokt.v002 != None and pat.sluchay.iddokt.v002 != '' else 0
        else:
            v002 = int(pat.le_vr.kod.v002) if pat.le_vr.kod and pat.le_vr.kod.v002 != None and pat.le_vr.kod.v002 != '' else 0
    ...
    def iddokt_sp(self,pat):
        if pat.sluchay.iddokt != None:
            return  f'720002 {pat.sluchay.iddokt.t005}'
        else:
    ...
    def ds1_sp(self,pat):
        if pat.sluchay.dskz2 != None:
            ...
            return ''
    def ds1_ksg2(self,pat):
        if pat.sluchay.dskz3:
            return pat.sluchay.dskz3.kod
        return ''
    ...
    def insert(self,pat,n):
        ...
        try:
            resdict_S['SMO_NAM'] = pat.vds.ctkom.naim_n[:100] if pat.vds != None and pat.vds.ctkom != None else None
        except:
            resdict_S['SMO_NAM'] = ''
        ...
        if pat.sluchay.iddokt:
            resdict_S['PROFIL'] = pat.sluchay.iddokt.v002
        else:
        ...
        resdict_S['DS3'] = pat.sluchay.ds_osl.kod if pat.sluchay.ds_osl != None else None
        ...
        if pat.sluchay.iddokt:
            resdict_S['PRVS'] = int(pat.sluchay.iddokt.v021) if pat.sluchay.iddokt.v021 != None and pat.sluchay.iddokt.v021 != '' else None
        else:
            resdict_S['PRVS'] = int(pat.le_vr.kod.v021) if pat.le_vr != None and pat.le_vr.kod != None  and pat.le_vr.kod.v021 != None and pat.le_vr.kod.v021 != '' else None
        ...
        if pat.sluchay.tip_oms == '1':
            if pat.sluchay.code_usl != None:
                if '1.1.1.1' not in pat.sluchay.code_usl.kod and  int(pat.sluchay.code_usl.kod[6]) == 1:
                    resdict_S['VID_VME'] = ''
                else:
        ...
        resdict_S['DOCORG'] = pat.patient.docorg[:100] if pat.patient.docorg else ''
        ...
        if opers is not None:
            for o in opers:
                kod = o.kod_op.kod if o.kod_op else None
                vra = None
                if o.kodan:
                    vra = o.kodan
                else:
                ...
                if kod in self.vid_vme_dem:

                    resdict_S_150 = copy.deepcopy(resdict_S)
                    resdict_S_150['CODE_USL'] = '1.1.3.150'
                    resdict_S_150['P_CEL'] = '2.6'
                    resdict_S_150['COMENTSL'] = '4'
                    resdict_S_150['IDSP'] = 28
                    resdict_S_150['VID_VME'] = kod
                    resdict_S_150['N_KSG'] = ''

                    resdict_S_150['TARIF'] = 3192.41
                    resdict_S_150['SUMV'] = 3192.41
                    resdict_S_150['KOEF_Z'] = None
                    resdict_S_150['KOEF_UP'] = None
                    resdict_S_150['BZTSZ'] = None
                    resdict_S_150['KOEF_D'] = None
                    resdict_S_150['KOEF_U'] = None
                    resdict_S_150['VIDPOM'] = 13

                    resdict_S_150['PROFIL'] = vra.v002 if vra else None
                    resdict_S_150['PRVS'] = int(vra.v021) if vra else None


                    vra_kod = vra.t005 if vra else ''
                    resdict_S_150['IDDOKT'] = f'720002 {vra_kod}'
                    resdict_S_150['CODE_MD'] = f'720002 {vra_kod}'

                    resdict_S_150['RSLT'] = 304
                    ishod = str(resdict_S_150['ISHOD'])
                    if len(ishod) > 0:
                        ishod = f'3{ishod[1:]}'
                        resdict_S_150['ISHOD'] = int(ishod)

                    resdict_S_150['C_ZAB'] = 1
                    resdict_S_150['PODR'] = '172000200017'
                    resdict_S_150['CODE_MES1'] = ''
                    resdict_S_150['CODE_MES2'] = ''
                    resdict_S_150['KD'] = None
                    resdict_S_150['KD_Z'] = None
                    resdict_S_150['COMENTU'] = None
                    resdict_S_150['NPL'] = None
                    resdict_S_150['CRIT'] = None
                    resdict_S_150['PROFIL_K'] = None
                    resdict_S_150['P_PER'] = None
                    resdict_S_150['PRIV'] = None



                    self.reestr.append(resdict_S_150)
        if '1.1.1.1' in code and vid_vme != '':
            resdict_U = copy.deepcopy(resdict_S)
            resdict_S['VID_VME'] = ''
            resdict_S['CODE_MD'] = ''
            resdict_U['BLOCK_CD'] = 'U'
            resdict_U['CODE_MD'] = f'720002 {pop.kodx.t005}' if pop and pop.kodx else ''
            resdict_U['PRVS'] = int(pop.kodx.v021) if pop.kodx and pop.kodx.v021 != None and pop.kodx.v021 != '' else None
            resdict_U['PROFIL'] = pop.kodx.v002 if pop.kodx != None  else None
            # if opers is not None:
            #     for o in opers:
            #         kod = o.kod_op.kod if o.kod_op else None
            pop_kod = pop.kod_op.kod if pop and pop.kod_op != None else None
            if pop_kod not in self.vid_vme_dem:
        ...
        if pat.sluchay.ksg_osn2:
            resdict_K = copy.deepcopy(resdict_S)
            resdict_K['BLOCK_CD'] = 'K'
            tarif = self.tarif_sp(pat,ksg2=True)
         
            resdict_K['DS1'] = self.ds1_ksg2(pat)
            resdict_K['CRIT'] = pat.sluchay.oopkk2.kod if pat.sluchay.oopkk2 else None
            resdict_K['CODE_MES1'] = pat.sluchay.ksg_osn2.ksg if pat.sluchay.ksg_osn2 != None else None
            resdict_K['TARIF'] = tarif.Tarif if tarif else None
            resdict_K['SUMV'] = tarif.Tarif if tarif else None
            resdict_K['CODE_USL'] = pat.sluchay.code_usl2.kod if pat.sluchay.code_usl2 else ''
            resdict_K['N_KSG'] = pat.sluchay.ksg_osn2.ksg if pat.sluchay.ksg_osn2 else ''
            resdict_K['KOEF_Z'] = tarif.KOEF_Z if tarif else None
            resdict_K['KOEF_UP'] = tarif.Koef_Up if tarif else None
            resdict_K['BZTSZ'] = tarif.BZTSZ if tarif else None
            resdict_K['KOEF_D'] = tarif.KOEF_D if tarif else None
            resdict_K['KOEF_U'] = tarif.KOEF_U if tarif else None
            self.reestr.append(resdict_K)
    ...
    def create_dbf(self):
        ...
        name = str(self.type_reestr).split('.')[0]
        if name == 'Реестр по выбранным историям':
            name = 'Реестр'
        self.dir += name +'_'+ str(self.user.user.id)
