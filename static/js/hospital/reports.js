...
Vue.component('all_filter', {
    template: `<div id="menu-list">
        <ul id="filters_ul">
          ...
          <li class="list-group-item"><input type="checkbox" id="ds_osl_filt">Ds осложнения</li>
          <li class="list-group-item"><input type="checkbox" id="ds_onk_filt">Ds онкология</li>
...
function get_checked_filters() {
    ...
    for (div of filters.childNodes) {
        if (div.id == 'datv_datp_filt') {
            ...
            filter.dsc = {
                dsc_1: div.childNodes[2].value,
                dsc_2: div.childNodes[4].value
            }
        }
        else if (div.id == 'dspat_filt_block') {
            filter.dspat = {
                // dspat: div.childNodes[1].value,
                dspat_1: div.childNodes[2].value,
                dspat_2: div.childNodes[4].value
            }
        }
        else if (div.id == 'ds_osl_filt_block') {
            filter.ds_osl = {
                // dson: div.childNodes[1].value,
                ds_osl_1: div.childNodes[2].value,
                ds_osl_2: div.childNodes[4].value
            }
        }
        else if (div.id == 'ds_onk_filt_block') {
            filter.ds_onk = {
                ds_onk_1: div.childNodes[2].value,
                ds_onk_2: div.childNodes[4].value
            }
        }
            ...
            filter.disability = {
                disability: div.childNodes[1].value,
            }
        }
        ...
        //     // }
        // }
        else if (div.id == 'rasxp_filt_block') {
            filter.rasxp = {
                rasxp: div.childNodes[1].value
            }
        }
    }
...
function create_reports(event) {
    ...
    else if (type_report == 'annual_14_4') { annual_14_4_f(type_report) }
...
function mix_reports() {
    ...
    let fil = document.getElementById("filter_fam_dato")
    let fil_child = fil.childNodes.length
    ...
    if (fil_child > 0){
        formData.append('fil_child',fil.childNodes[1].value)
    }
...
function a_oth_29_f(type_report) {
    ...
    var filter = {}
    filter.dskz = {
        dskz_1: input_ds_1.value,
        dskz_2: input_ds_2.value
    }
...
function annual_14_4_f(type_report){
    let date_1 = $("#statistics_rep").find("#date1")
    let date_2 = $("#statistics_rep").find("#date2")
    var formData = new FormData()
    formData.append('task_type', 'annual')
...
function onchange_group_p_list() {
    ...
    if (select.value == 'group_p1') {
        ...
        let data = ['По фамилии','По Дате выписки']
...
function onchange_panel_14_menu(event) {
    ...
    setTimeout(() => {
        ...
        document.getElementById("statistics_rep_btn").setAttribute('btn', 'annual_'+val)
    }, 200)
}
