...
class Query(graphene.ObjectType):
    ...
    def resolve_V012(*args, **kwargs):
        if 'V012_hospital' in cache:
            ...
            cache.set('V012_hospital', V012.objects.filter(id_iz__istartswith='1',dateend=None).order_by('iz_name').all(), timeout=60 * 360)
            return V012.objects.filter(id_iz__istartswith='1',dateend=None).order_by('iz_name').all()

    def resolve_V009(*args, **kwargs):
        if 'V009_hospital' in cache:
            return cache.get('V009_hospital')
        else:
            cache.set('V009_hospital', V009.objects.filter(id_tip__istartswith='1',dateend=None).order_by('tip_name').all(),timeout=60 * 360)
            return V009.objects.filter(id_tip__istartswith='1',dateend=None).order_by('tip_name').all()
    ...
    def resolve_V036_list(self,*args, **kwargs):
        if 'V036_list_hospital' in cache:
            ...
            cache.set('V036_list_hospital', V036.objects.filter(dateend=None,parameter__in=[1,3]).all(), timeout=60 * 360)
            return V036.objects.filter(dateend=None,parameter__in=[1,3]).all()
