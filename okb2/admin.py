...
from .models import (Statistics_type, F008, V006, V008, V014, F003, V009, V012, V010, V002, V020, V025, V021,
    ...
    Code_med_dev,UpdatePers,UpdatePersData,Tar_vt,V036,Tarif,vbb_ds_prk)
    ...
    def upload(self,request):
        if request.method == "POST":
            ...
            for rownum in range(sheet.nrows):
                row = sheet.row_values(rownum)
                if rownum > 0:
                    vra_count = Vra.objects.filter(kod=str(row[0]).strip()).count()
                    if vra_count == 0:
                        v = Vra()
...
class F003Admin(admin.ModelAdmin):
    search_fields = ('naim',)
    list_display = ('kod','naim')
admin.site.register(F003,F003Admin)
...
admin.site.register(vbb_ds_prk)